...
export function SendNotificationModal({
  ...
  async function handleConfirmSend() {
    ...
    try {
      ...
      const res = await fetch('/api/notifications/send/drivers/targeted', {
        method: 'POST',
        headers: {
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-3 px-6 pt-6 pb-3 border-b border-gray-100">
          <div>
            ...
            >
              Send notification to filtered drivers
            </h2>
            <p className="mt-1 text-sm text-gray-600">
              ...
              {totalDriversShown === 1 ? 'driver' : 'drivers'} matched by the
              current filters. Only drivers with a registered FCM token will
          ...
          <div className="px-6 py-6 space-y-5">
            <div className="rounded-lg bg-amber-50 border border-amber-100 px-4 py-3 text-sm text-amber-900">
              <p className="font-semibold">
                ...
                {recipientCount === 1 ? 'driver' : 'drivers'}?
              </p>
              <p className="mt-1">
                This pushes immediately to every driver in the current filtered
